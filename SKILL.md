...
4. Prepare prompt text, reference audio, and optionally a reference image.
5. Treat the packaged App V3 as audio-driven by default. Uploaded audio is on the active path, and `USE ONLY VOCALS` is App-exposed.
6. For text-to-video, flip `Text To Video (no image ref)` to `true`, but still keep an image available because the notebook explicitly warns that loading an image helps avoid errors.
7. Keep first smoke runs short, with moderate size and a fixed seed.
8. Validate the produced MP4, audio, lip-sync, and actual output size or duration before claiming the workflow is working.
